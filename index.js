...
app.get('/', (req, res) => { res.send('🚀 AI Haggle Pro Active'); });
app.listen(PORT, () => {
  console.log(`📡 Порт: ${PORT}`);
  initBot().catch(e => console.error(e.message));
});

console.log('⚡️ Режим AI HAGGLE PRO!');
const storage = new Map(), userStates = new Map(), browsers = new Map();
...
async function startAvitoAuth(uid, phone) {
  ...
  if (!isLocal) {
    const cleanProxy = process.env.PROXY_SERVER.replace(/^https?:\/\//, '');
    args.push(`--proxy-server=http://${cleanProxy}`);
    args.push('--host-resolver-rules=MAP * ~NOTFOUND , EXCLUDE 127.0.0.1');
  }

  const b = await pt.launch({ headless: true, args, env: { LANG: 'ru_RU.UTF-8' } });
  ...
  if (!isLocal) {
    try {
      await p.goto('https://ipify.org', { waitUntil: 'networkidle2', timeout: 15000 });
      const currentIp = await p.evaluate(() => document.body.innerText);
      console.log(`📡 [PROXY CHECK] Авторизация сессии ${uid} запущена через IP: ${currentIp.trim()}`);
    } catch (e) {
      console.log(`⚠️ [PROXY WARNING] Не удалось проверить IP-адрес перед авторизацией: ${e.message}`);
    }
  }
  ...
  if (!await p.$(smsSel)) {
    ...
    if (t.includes('капча') || t.includes('робот')) throw new Error('Капча! Защита Авито заблокировала серверный запрос.');
...
async function finishAvitoAuth(uid, code) {
  ...
  try {
    ...
    const ok = ck.some(c => c.name.includes('sessid') || c.name.includes('u'));
    if (ok) {
      await db.collection('user_sessions').doc(String(uid)).set({ cookies: ck, updatedAt: new Date() });
      return true;
    } else {
      throw new Error('Код отклонен.');
    }
  } catch (e) {
    throw e;
  } finally {
  ...
  try {
    ...
    b = await pt.launch({ headless: true, args, env: { LANG: 'ru_RU.UTF-8' } });
    const p = await b.newPage();
    await optimizePage(p);
    ...
    if (!isLocal) {
      try {
        await p.goto('https://ipify.org', { waitUntil: 'networkidle2', timeout: 15000 });
        const currentIp = await p.evaluate(() => document.body.innerText);
        console.log(`📡 [PROXY CHECK] Работа сессии ${uid} через IP: ${currentIp.trim()}`);
      } catch (e) {
        console.log(`⚠️ [PROXY WARNING] Не удалось проверить IP-адрес перед торгом: ${e.message}`);
      }
    }
...
async function initBot() {
  ...
  if (!t) { console.error('❌ Нет токена!'); return; }
  ...
  const bot = new Telegraf(t), limiter = new Bottleneck({ maxConcurrent: 1, minTime: 1500 });
  ...
  bot.action('view_stats', async (ctx) => {
    ...
    } catch (e) { await ctx.reply('❌ Ошибка синхронизации данных.'); }
  });
  ...
  bot.launch().then(() => console.log('🚀 Бот успешно запущен в Telegram!'));
}
bot.action(/^send_bid_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const bidId = ctx.match[1], uid = ctx.from.id.toString();
...
bot.on('text', async (ctx) => {
  const text = ctx.message.text.trim(), uid = ctx.from.id.toString(), state = userStates.get(uid);

  // Шаг 1: Ожидание ввода номера телефона
  if (state?.step === 'PHONE') {
    if (!/^\d{11}$/.test(text)) return ctx.reply('❌ Некорректный формат. Нужно ровно 11 цифр (например, 79991112233):');
    ...
    try {
      ...
      userStates.delete(uid);
      // Гарантированная очистка зависших инстансов браузера при аварийном выходе
      if (browsers.has(uid)) {
        try { await browsers.get(uid).browser.close(); } catch(e) {}
        browsers.delete(uid);
      }
  ...
  if (state?.step === 'SMS') {
    ...
    try {
      ...
      userStates.delete(uid);
      await ctx.reply(`❌ Ошибка проверки кода: ${err.message}`);
    }
    return;
  }

  // Шаг 3: Обработка ссылок Авито и запуск DeepSeek
  if (/avito\.ru/i.test(text) && (text.includes('http://') || text.includes('https://'))) {
    const check = await db.collection('user_sessions').doc(uid).get();
    if (!check.exists) return ctx.reply('⚠️ Защищенный шлюз закрыт. Сначала авторизуйте Авито кнопкой из панели.');
    ...
    try {
      const comp = await openai.chat.completions.create({
        ...
        messages: [
          ...
          { role: "user", content: `Сделай торг для товара по ссылке: ${text}` }
        ],
        response_format: { type: "json_object" }
      });

      let rawJson = comp.choices[0].message.content;
      rawJson = rawJson.replace(/```json|```/g, "").trim();
      const ai = JSON.parse(rawJson);

      const est = Number(ai.estimatedPrice) || 0, trg = Number(ai.targetPrice) || 0, arg = ai.argument || '';
      const profit = est > trg ? (est - trg) : 0, comm = Math.round(profit * 0.3);

      // Корректное создание одной записи в истории сделок без дублирующих затираний
      ...
      console.error(err);
      await ctx.reply('⚠️ Ошибка нейро-скоринга ИИ или сбой парсинга ответа.');
    }
  } else {
    await ctx.reply('Пожалуйста, отправьте валидную веб-ссылку на товар с Авито.');
  }
});

// Настройка вебхука и роутинга Express
app.post(TG_PATH, (req, res) => { bot.handleUpdate(req.body, res); });

try {
  await bot.telegram.setWebhook(`${APP_URL}${TG_PATH}`);
  console.log(`[Telegram] Вебхук успешно активирован на: ${APP_URL}${TG_PATH}`);
} catch (e) {
...
} // Конечная закрывающая скобка функции initBot
